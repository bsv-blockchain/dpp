# Legacy UORA-named native claims and anchor v3

**Status: historical native format.** These rules apply only to previously issued native claims without `claimFormat` and `uora-anchor-v3` outputs. They do not specify a full UORA credential or VSC SEAL. New writes use [the current rules](rules.md). Historical bytes are never renamed or reinterpreted.
...
**The two-names rule.** A historical payload may nominate a resolvable DID and a `did:key` as related identifiers. That relationship requires independent authenticated evidence; it is not established by the payload. Their roles in the encoding are: the resolvable name in `issuer` for tooling that can reach a resolver, the offline-decodable name in `issuerKeyDid` and in the anchor's issuer field for a reader holding one transaction and nothing else.
...
**An anchor is a leaf.** Historical writers intended to leave it unspent, but the script permits the locking key to spend it. Spending does not revoke the attestation or erase its original commitment. The anchor carries a digest and public metadata, not the attestation body.
