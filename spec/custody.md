...
**Version 2.** Record version 2 carries this predicate on the wire for every non-genesis operation, not only for `TRANSFER` and not only under a profile: field 14 of [`record-model-v2.md`](record-model-v2.md) is the scalar, the evaluation order is the one above, and the transfer authorities become control authorities that may also make an `UPDATE` or a `RETIRE`. The scalar, the derivation and the wallet operation that reveals it are unchanged, so a wallet that can prove a version 1 transfer can prove a version 2 state. [`managed-custody.md`](managed-custody.md) defines the custody profile version 2's authorisation commitment exists for: a custodian holding the keys for parties who have no wallet, with the recipient's acceptance recorded, signed and committed to on chain.
