...
An owner is named differently from a business, and on purpose. The key in field 6 is a per-passport child of the owner's wallet root with counterparty `self` ([`record-model.md`](record-model.md) §3), so it is pseudonymous: it resolves to no DID a stranger can look up, and the same owner's other passports show unrelated keys. The owner's identity is the root itself, and it attaches to a passport off chain, when the owner chooses, either by signing a challenge under the root with `createSignature` or by revealing the specific key linkage of that one passport to the asking party under BRC-69. When the owner acts, their root is field 7 and has a `did:key` like any actor's; [`custody.md`](custody.md) §7 says what follows from that.
