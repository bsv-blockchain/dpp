...
|---|---|---|---|---|
| **Passport reader** | Decode and verify supplied token history and report the scope of the finding | Transaction bytes; a header source or the explicit decision to skip inclusion | `examples/verify-passport.mjs --fixture`: no account, no wallet, no other role | `fixtures/record-v1.json`, `fixtures/chain-v1.json`, the token checks of `fixtures/evidence-v1.json`; under `native-baseline@2` also `fixtures/record-v2.json`, `fixtures/chain-v2.json`, `fixtures/managed-acceptance-v1.json` and `fixtures/evidence-v2.json` |
...
[`../conformance/baseline-native-2.json`](../conformance/baseline-native-2.json) is `native-baseline@2`: the same selection with record version 2 ([`record-model-v2.md`](record-model-v2.md)) as the current record, the version 2 derivation beside the version 1 one, the managed acceptance record of [`managed-custody.md`](managed-custody.md) as a representation, and the version 2 fixtures beside the version 1 ones. A component under `native-baseline@2` reads version 1 states as `native-baseline@1` requires and version 2 states as the second document requires, because a lineage may hold both; `native-baseline@1` remains the baseline for a component that reads version 1 alone, and neither supersedes the other until a version 1.0 declaration says so.
