...
Passport admission checks the native state under its own record version and the selected custody policy. A version 2 state is admitted only when its lineage genesis can be traced, through the submitted bytes or the index's own admitted history, so that `lineage_genesis` is checked and never assumed; a state that spends a version 2 `RETIRE` is never admitted and the retired tip stays in place; a genesis that spends an admitted passport output is refused; and an index declaring `managed-custody@1` refuses a version 2 `TRANSFER` without its acceptance commitment. Generic anchor admission checks its exact script, framed metadata signature and anchoring-service key derivation. It does not validate a VSC credential that is not present in the transaction. Returned BEEF enables independent transaction and inclusion verification; an index's response is not a global completeness or latest-state proof.
