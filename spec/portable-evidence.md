...
A single package MAY be bounded, and a bounded package declares it: the states it does not carry are absent by outpoint and its snapshot declaration is not complete. Completeness for a lineage longer than the bound is reached by the complete export, `dpp-evidence-export@1` ([`../contracts/evidence-export.schema.json`](../contracts/evidence-export.schema.json)): parts over one snapshot of one source, each a package of its own over a contiguous range of the source's sequence, oldest first, each bounded by a number of states and a number of transaction and proof bytes with at least one state per part, and resumed by an opaque cursor under the rules of §1. Each part carries a coverage record naming the passport, the snapshot (source, identifier and sequence), the part's index, the range it covers, its number of states, whether it is final, and the SHA-256 of the part's package manifest exactly as signed; the exporter signs the record with the same key as the manifest, under the same preimage rule, and the cursor stays outside it, being a resumption token and no statement about coverage. Consecutive parts tile the snapshot from its first sequence to the snapshot's own sequence with no gap and no overlap, the final part ends at the snapshot's sequence and carries a null cursor, and a part's manifest is complete for its own range and never for the snapshot, unless it is the only part; the manifest's scope declaration and the coverage record say one thing, and a reader refuses a part where they differ. A part carries no verification report: the report is the reader's to produce over the joined history. A reader establishes coverage only from records whose signature verifies under the exporter's key and whose digest binds the package it received; an unsigned wrapper field establishes nothing. It then joins the parts by those records, refusing by name a repeated or missing part, a part from another snapshot or another passport, a part signed by another key, a package the record does not bind, a part whose package does not inspect clean, a path repeated with different bytes, or a final part that does not end at the snapshot's sequence; only then does it hold what the exporter asserted the snapshot held, and it still verifies every transaction, proof and claim inside. The signature attributes the coverage assertion to the exporter; it does not prove that the exporter held everything that exists elsewhere, which remains the exporter's responsibility for its snapshot. The exporter bounds each part and MAY require authentication for the complete export while leaving the bounded package and the pages open; its capability document states the bounds and which it does. A package produced before the complete export existed is read exactly as it was: bounded, and saying so.
...
What a conforming party reproduces: the page properties and their meanings, the snapshot ordering and the cursor and expiry rules (§1); the package manifest, its inventory categories, the digest-first import and the declarations of absence, and the composition of a complete export from parts that tile one snapshot (§2); and the separation of blockchain acceptance from durable publication (§3). What is a build's own: the storage behind a snapshot, the archive form, the bounds of a part and whether the complete export is authenticated, where a replica runs, and how a deployment meets the objectives it declares.
