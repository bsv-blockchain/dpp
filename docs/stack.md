...
The stack has a settled shape for a token standard's code, and the Mandala regulated-stablecoin work ([`bsv-blockchain/mandala`](https://github.com/bsv-blockchain/mandala)) is the most recent example of it: the script templates live in `@bsv/templates` as `ScriptTemplate` classes with `lock`, `unlock` and a static `decode` (`packages/helpers/ts-templates/src/MandalaToken.ts`, `MandalaAdmin.ts`), the topic manager and lookup service live in `@bsv/overlay-topics` beside every other canonical topic (`packages/overlays/topics/src/mandala/`), both are published to npm, and the application and the overlay deployment consume them as versioned dependencies pinned to the same version so that writer and reader cannot drift. A Go port of the overlay is checked against vectors generated from the TypeScript reference. Nothing in Mandala's token semantics applies to a passport; its delivery shape does, and [`stack-review.md`](stack-review.md) §6 records what the reference implementation here should borrow from it.
