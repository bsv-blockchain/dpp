...
- **This repository, `fixtures/vectors`.** The record and anchor vector files were generated once and are held by hand; the chain's are produced by `packages/dpp-core/test/chain-v1-vectors.ts` and held identical by a test. Give the other two the same generator so no vector file is ever edited directly.
...
- **dpp-app, the identifier.** `EXAMPLE_GTIN` in `packages/dpp-service/src/digital-link.ts` is `09506000134352` and `PASSPORT_ID_ROOT` is `id.gs1.org`, a host that will never answer for a demonstration number. Mint one 952 GTIN per demonstration model under `dpp.bsvb.net`, as `record-model.md` §3 requires and [`identifiers.md`](identifiers.md) explains; states already published under the old number stay as they are, because a state is permanent.
