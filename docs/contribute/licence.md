...
The repository and its four packages use Open BSV License Version 6, reproduced from the Teranode licence linked below. Use is restricted to the BSV Blockchain as defined by that licence. Copies or substantial portions must include the licence and the text “© BSV Association”. Third-party artefacts retain their own terms and notices.
...
The source repository is public. Initial experimental beta packages are available from npm; the beta.2 package update is pending. See [release status](../reference/release-sets.md).
