...
| `EXPORT_SIGNING_KEY` | Private key, hex, that signs `dpp-evidence-package@1` manifests served by `GET /evidence-package` (`spec/portable-evidence.md` §2). A key of its own, never the publisher key: it says which node assembled a package, nothing about admission. Malformed, it stops the boot. | `GET /evidence-package` answers 503 `export-unavailable` and the capability document lists the export as unsupported |
...
What synchronises, and what does not, with the SDK as it stands (`@bsv/overlay` 2.0.3, `@bsv/gasp`):
